...
\block{Tensor Train Representation of $[\theta|y]$}{\vspace*{-2ex}
\centering
A surface rendering of a density in TT format \\ TT formula
\centerline{\includegraphics[]{PosterMargTT.png}}
...
\block{Inverse Problem of Recovering Ozone Profile }{
	...
	\begin{minipage}[b]{0.45\linewidth}
		\centerline{\input{LIMBPoster.pdf_tex}}
...
\begin{minipage}[b]{0.54\linewidth}
		...
		\node[roundnode2] at (-6.75,6.5) (S)    {$\bm{\Sigma}$};
		\node[roundnode2] at (-8.5,9) (s)    {$\gamma$};
		\node[roundnode2] at (-6.25,9) (d)    {$\delta$};
		\node[roundnode2] at (3,6.5) (t)     {$\bm{T}$};
		\node[roundnode2] at (-1,6.5) (p)     {$\bm{p}$};
		\node[roundnode2] at (1,4) (pt)     {$\bm{p}/\bm{T}$};
		\node[roundnode2] at (1,9) (b1)    {$b$};
		%\node[roundnode2] at (1,8) (b2)    {$b_2$};
		\node[roundnode2] at (-3.75,9) (h1)    {$h_{0}$};
		\node[roundnode2] at (-1.25,9) (p0)    {$p_0$};
		\node[roundnode2] at (3.5,9) (ht)    {$\bm{h}$};
		\node[roundnode2] at (5.75,9) (ct)    {$T_0$};
		\node[roundnode2] at (8,9) (at)    {$\bm{a}$};
